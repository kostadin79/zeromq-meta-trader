...
extern string PROJECT_NAME = "MetaTrader 4 Bridge for ZeroMQ Nest";
...
void SendUpdateMessage(Socket &pSocket) {
	...
	// @TODO add property REQUEST_CHANGE_UPDATE_PERIOD
	// if (seconds % 10 == 0 && lastUpdateSeconds != seconds) {
...
void InterpretZmqMessage(Socket& pSocket, string& compArray[]) {
	...
	if (apiKey != APIKEY) {
		...
		if (ArraySize(compArray) != 4) {
			...
			string symbol = compArray[3];
			string symbolRates = GetRatesString(symbol);
			response = StringFormat("%d|%s", RESPONSE_OK, symbolRates);
		}
	} else if (type == REQUEST_MULTIPLE_RATES) {
      string symbols[];
      string changes[];
		string sep = ",";
		ushort u_sep = StringGetCharacter(sep, 0);
    	...
    	for(int i = 0; i < ArraySize(symbols); i++) {
			string rateString = GetRatesString(symbols[i]);
			ArrayPush(changes, rateString);
		}
		...
		bool opened = ChartOpen(compArray[3], PERIOD_D1) != 0;
		if (opened) {
			Sleep(100);
			MqlRates rates[];
			int  a = ArrayCopyRates(rates, compArray[3], PERIOD_D1);
			Print("all->",a);
			closeAllChart();
